...
export class CodeTransformer {
   ...
   */
  public transformPercy(sourceCode: string): CodeTransformationResult {
   ...
   */
  private transformPercyImport(path: any, warnings: TransformationWarning[]): void {
    const source = path.node.source.value;
    
    // Map Percy SDK imports to SmartUI equivalents
    const importMappings: { [key: string]: string } = {
   ...
   * @param warnings - Array to add warnings to
   */
  private transformPercyRequire(path: any, warnings: TransformationWarning[]): void {
    if (path.node && path.node.arguments && path.node.arguments.length > 0 && t.isStringLiteral(path.node.arguments[0])) {
      ...
      const requireMappings: { [key: string]: string } = {
        '@percy/cypress': '@lambdatest/smartui-cypress',
        '@percy/playwright': '@lambdatest/smartui-playwright',
        '@percy/storybook': '@lambdatest/smartui-storybook',
        '@percy/selenium-webdriver': '@lambdatest/smartui-selenium',
   ...
   * @param warnings - Array to add warnings to
   */
  private transformPercySnapshot(path: any, warnings: TransformationWarning[]): void {
   ...
   */
  private transformPercySnapshotOptions(optionsNode: any, warnings: TransformationWarning[], snapshotPath: any): void {
    const newProperties: any[] = [];
    let hasWidthsWarning = false;

    optionsNode.properties.forEach((prop: any) => {
        ...
        switch (key) {
          ...
          case 'ignore_region_selectors':
            ...
            if (t.isArrayExpression(prop.value)) {
              newProperties.push(
                t.objectProperty(
                  t.identifier('ignoreDOM'),
                  t.objectExpression([
                    t.objectProperty(
                      t.identifier('cssSelector'),
                      prop.value
                    )
                  ])
                )
   ...
   */
  private addWarningComment(snapshotPath: any): void {
    const warningComment = t.addComment(
      snapshotPath.node,
      'leading',
      ' MIGRATION-WARNING: per-snapshot widths are not supported. Please configure viewports in your .smartui.json file.',
      false
    );
  }
   ...
   * @returns CodeTransformationResult - Transformation result with content, warnings, and snapshot count
   */
  public transformApplitools(sourceCode: string, framework: 'Cypress' | 'Playwright'): CodeTransformationResult {
    const warnings: TransformationWarning[] = [];
    let snapshotCount = 0;
   ...
   * @param warnings - Array to add warnings to
   */
  private transformApplitoolsImport(path: any, warnings: TransformationWarning[]): void {
    const source = path.node.source.value;
   ...
   * @param warnings - Array to add warnings to
   */
  private transformApplitoolsRequire(path: any, warnings: TransformationWarning[]): void {
    if (path.node && path.node.arguments && path.node.arguments.length > 0 && t.isStringLiteral(path.node.arguments[0])) {
      const source = path.node.arguments[0].value;
   ...
   */
  private isEyesOpenCall(callNode: any): boolean {
    return (
   ...
   */
  private isEyesCloseCall(callNode: any): boolean {
    return (
      (t.isMemberExpression(callNode.callee) &&
       t.isIdentifier(callNode.callee.object, { name: 'eyes' }) &&
   ...
   */
  private isEyesCheckCall(callNode: any): boolean {
    return (
      (t.isMemberExpression(callNode.callee) &&
       t.isIdentifier(callNode.callee.object, { name: 'eyes' }) &&
   ...
   */
  private transformEyesCheckToSmartUISnapshot(callNode: any, warnings: TransformationWarning[], framework: 'Cypress' | 'Playwright'): void {
   ...
   */
  private parseEyesCheckArguments(callNode: any, warnings: TransformationWarning[]): { snapshotName: string, options: any, isLayout: boolean } {
   ...
   * @param warnings - Array to add warnings to
   */
  private parseApplitoolsOptions(optionsNode: any, options: any, warnings: TransformationWarning[]): void {
    optionsNode.properties.forEach((prop: any) => {
      if (t.isObjectProperty(prop) && t.isIdentifier(prop.key)) {
        const key = prop.key.name;
   ...
   */
  private createSmartUISnapshotCall(snapshotName: string, options: any, framework: 'Cypress' | 'Playwright'): any {
    ...
    if (Object.keys(options).length > 0) {
        ...
        optionsObject.properties.push(
          t.objectProperty(
            ...
            t.objectExpression([
              t.objectProperty(
                t.identifier('cssSelector'),
                t.arrayExpression(options.ignoreDOM.cssSelector.map((selector: string) => t.stringLiteral(selector)))
              )
            ])
          )
        );
        ...
        optionsObject.properties.push(
          t.objectProperty(
            ...
            t.objectExpression([
              t.objectProperty(
                t.identifier('cssSelector'),
                t.stringLiteral(options.element.cssSelector)
              )
            ])
          )
        );
      ...
      args.push(optionsObject as any);
    }
   ...
   * @param warnings - Array to add warnings to
   */
  private injectLayoutEmulation(originalCall: any, smartUISnapshotCall: any, framework: 'Cypress' | 'Playwright', warnings: TransformationWarning[]): void {
    ...
    const warningComment = t.addComment(
      originalCall,
      'leading',
      ' MIGRATION-NOTE: Applitools \'layout\' region was emulated. A functional assertion was added to check for the container\'s visibility, and a SmartUI snapshot was taken with child elements ignored. Please verify this provides adequate coverage.',
      false
    );

    // Create functional assertion based on framework
    let assertionCall: any;
    if (framework === 'Playwright') {
      assertionCall = t.awaitExpression(
        t.callExpression(
          t.memberExpression(
            t.callExpression(
              t.memberExpression(t.identifier('expect'), t.identifier('toBeVisible')),
              [t.callExpression(
                t.memberExpression(t.identifier('page'), t.identifier('locator')),
                [t.stringLiteral(layoutSelector)]
              )]
            ),
            t.identifier('toBeVisible')
          ),
          []
        )
      );
      ...
      // Cypress
      assertionCall = t.callExpression(
        t.memberExpression(
          t.callExpression(
            t.memberExpression(t.identifier('cy'), t.identifier('get')),
            [t.stringLiteral(layoutSelector)]
          ),
          t.identifier('should')
        ),
        [t.stringLiteral('be.visible')]
      );
    }
        ...
        optionsArg.properties.push(
          t.objectProperty(
            ...
            t.objectExpression([
              t.objectProperty(
                t.identifier('cssSelector'),
                t.arrayExpression([t.stringLiteral(`${layoutSelector} *`)])
              )
            ])
          )
        );
   ...
   * @returns CodeTransformationResult - Transformation result with content, warnings, and snapshot count
   */
  public transformSauceLabs(sourceCode: string): CodeTransformationResult {
    const warnings: TransformationWarning[] = [];
    let snapshotCount = 0;
   ...
   * @param warnings - Array to add warnings to
   */
  private transformSauceLabsImport(path: any, warnings: TransformationWarning[]): void {
    const source = path.node.source.value;
   ...
   * @param warnings - Array to add warnings to
   */
  private transformSauceLabsRequire(path: any, warnings: TransformationWarning[]): void {
    if (path.node && path.node.arguments && path.node.arguments.length > 0 && t.isStringLiteral(path.node.arguments[0])) {
      const source = path.node.arguments[0].value;
   ...
   */
  private isSauceVisualCheckCall(callNode: any): boolean {
    return (
      (t.isIdentifier(callNode.callee, { name: 'sauceVisualCheck' })) ||
      (t.isMemberExpression(callNode.callee) &&
       t.isIdentifier(callNode.callee.object, { name: 'cy' }) &&
   ...
   * @param warnings - Array to add warnings to
   */
  private transformSauceVisualCheckToSmartUISnapshot(callNode: any, warnings: TransformationWarning[]): void {
   ...
   */
  private parseSauceVisualCheckArguments(callNode: any, warnings: TransformationWarning[]): { snapshotName: string, options: any } {
    let snapshotName = 'Untitled Snapshot';
    let options: any = {};
   ...
   * @param warnings - Array to add warnings to
   */
  private parseSauceLabsOptions(optionsNode: any, options: any, warnings: TransformationWarning[]): void {
    optionsNode.properties.forEach((prop: any) => {
      if (t.isObjectProperty(prop) && t.isIdentifier(prop.key)) {
        const key = prop.key.name;
   ...
   * @returns The AST node for the smartuiSnapshot call
   */
  private createSmartUISnapshotCallFromSauceLabs(snapshotName: string, options: any): any {
    const args = [t.stringLiteral(snapshotName)];
