...
exports.JavaCodeTransformer = void 0;
/**
...
class JavaCodeTransformer {
     ...
     */
    transformWithRegex(sourceCode, platform, warnings) {
     ...
     */
    transformImports(sourceCode, platform) {
        let transformedCode = sourceCode;
     ...
     * @returns The transformed code
     */
    transformPercyMethods(sourceCode) {
     ...
     * @returns The transformed code
     */
    transformApplitoolsMethods(sourceCode) {
        let transformedCode = sourceCode;
     ...
     * @returns The transformed code
     */
    transformSauceLabsMethods(sourceCode) {
     ...
     */
    countSnapshotsWithRegex(sourceCode, platform) {
        let count = 0;
        switch (platform) {
            case 'Percy':
                const percySnapshotRegex = /(\w+)\.snapshot\s*\(\s*"([^"]+)"\s*\)/g;
...
class JavaTransformationVisitor {
    ...
    visitCompilationUnit(node) {
        ...
        // Visit all children
        if (node.children) {
            transformedNode.children = node.children.map((child) => this.visit(child));
        }
    ...
    visitChildren(node) {
        ...
        for (const [key, value] of Object.entries(node.children)) {
            if (Array.isArray(value)) {
                transformedNode.children[key] = value.map((item) => this.visit(item));
            }
     ...
     */
    extractQualifiedName(qualifiedNameNode) {
     ...
     */
    shouldTransformImport(qualifiedName) {
     ...
     */
    createQualifiedNameNode(qualifiedName) {
     ...
     */
    isVisualTestingMethod(methodName, className) {
        // Debug logging
     ...
     * @returns The transformed method invocation
     */
    transformVisualTestingMethod(node) {
        const methodName = this.extractMethodName(node);
        switch (this.platform) {
            case 'Percy':
     ...
     * @returns The transformed method invocation
     */
    transformPercyMethod(node, methodName) {
     ...
     */
    transformApplitoolsMethod(node, methodName) {
     ...
     * @returns The transformed method invocation
     */
    transformSauceLabsMethod(node, methodName) {
     ...
     */
    createSmartUISnapshotCall(originalNode) {
     ...
     * @returns Array of argument nodes
     */
    createSmartUIArguments(snapshotName, options) {
        const args = [
            {
                name: 'Primary',
                children: {
     ...
     * @returns The transformed code
     */
    transformAppiumSpecificCode(sourceCode, platform, warnings) {
        let transformedCode = sourceCode;
     ...
     * @returns The transformed code
     */
    transformPercyAppiumCommands(sourceCode, warnings) {
        let transformedCode = sourceCode;
     ...
     */
    transformPercyAppiumOptions(options, warnings) {
        if (!options)
            return '';
     ...
     */
    preserveNativeContextSwitching(sourceCode, warnings) {
...
//# sourceMappingURL=JavaCodeTransformer.js.map