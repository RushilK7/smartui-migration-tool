...
class ConfigTransformer {
     ...
     */
    async transformConfigs(scanResult) {
     ...
     */
    transformSauceLabsConfig(fileContent, filePath) {
        const warnings = [];
        try {
            ...
            if (filePath.endsWith('.yml') || filePath.endsWith('.yaml')) {
                ...
                configObject = js_yaml_1.default.load(fileContent);
            }
     ...
     */
    async createSmartUIConfig(config) {
     ...
     * @returns ConfigTransformationResult - Transformation result with content and warnings
     */
    transformPercyConfig(fileContent) {
        const warnings = [];
        try {
            // Parse the Percy configuration (YAML or JSON)
            const percyConfig = js_yaml_1.default.load(fileContent);
            // Initialize SmartUI configuration
            const smartUIConfig = {
                version: '1.0',
                projectName: 'migrated-project',
            ...
            if (percyConfig.snapshot?.widths && Array.isArray(percyConfig.snapshot.widths)) {
                smartUIConfig.web.viewports = percyConfig.snapshot.widths.map((width) => [width]);
            }
     ...
     * @returns ConfigTransformationResult - Transformation result with content and warnings
     */
    transformApplitoolsConfig(fileContent) {
        const warnings = [];
        try {
     ...
     */
    extractObjectFromAST(objectExpression) {
        const result = {};
     ...
     */
    processBrowserArray(browserArray) {
        const webBrowsers = [];
        const webViewports = [];
        const mobileDevices = [];
     ...
     */
    handleNonMappableProperties(configObject, warnings) {
     ...
     */
    parseJavaScriptConfig(fileContent) {
        const ast = (0, parser_1.parse)(fileContent, {
            sourceType: 'module',
            allowImportExportEverywhere: true,
            allowReturnOutsideFunction: true,
            ...
            configObject.suites.forEach((suite) => {
                if (suite.settings) {
                    ...
                    if (suite.settings.browsers && Array.isArray(suite.settings.browsers)) {
                        suite.settings.browsers.forEach((browser) => {
                    ...
                    if (suite.settings.devices && Array.isArray(suite.settings.devices)) {
                        suite.settings.devices.forEach((device) => {
     ...
     * @param warnings - Array to add warnings to
     */
    processJavaScriptSauceConfig(configObject, smartUIConfig, warnings) {
        ...
        if (sauceConfig.browsers && Array.isArray(sauceConfig.browsers)) {
            sauceConfig.browsers.forEach((browser) => {
                if (browser.browserName && !smartUIConfig.web.browsers.includes(browser.browserName)) {
                    smartUIConfig.web.browsers.push(browser.browserName);
                }
                if (browser.screenResolution) {
        ...
        if (sauceConfig.devices && Array.isArray(sauceConfig.devices)) {
            sauceConfig.devices.forEach((device) => {
                if (device.deviceName) {
                    smartUIConfig.mobile.devices.push(device.deviceName);
                }
            });
     ...
     */
    parseScreenResolution(resolution) {
     ...
     * @param warnings - Array to add warnings to
     */
    handleSauceLabsNonMappableProperties(configObject, warnings) {
        const nonMappableProperties = [
            {
                key: 'build',
