...
exports.PythonCodeTransformer = void 0;
/**
...
class PythonCodeTransformer {
     ...
     */
    transform(sourceCode, filePath, platform) {
        const warnings = [];
        let snapshotCount = 0;
        try {
     ...
     */
    transformPythonFile(sourceCode, platform, warnings) {
     ...
     * @returns CodeTransformationResult
     */
    transformRobotFramework(sourceCode, platform, warnings) {
        let transformedCode = sourceCode;
        let snapshotCount = 0;
     ...
     */
    transformPythonImports(sourceCode, platform) {
        let transformedCode = sourceCode;
     ...
     */
    getPythonImportMappings(platform) {
        switch (platform) {
            case 'Percy':
                return {
     ...
     * @returns The transformed code
     */
    transformPercyPythonMethods(sourceCode) {
     ...
     * @returns The transformed code
     */
    transformApplitoolsPythonMethods(sourceCode) {
        let transformedCode = sourceCode;
     ...
     * @returns The transformed code
     */
    transformSauceLabsPythonMethods(sourceCode) {
     ...
     */
    countPercySnapshots(sourceCode) {
        const percySnapshotRegex = /(\w+)\.snapshot\s*\(\s*["']([^"']+)["']\s*\)/g;
     ...
     * @returns The number of snapshots found
     */
    countApplitoolsSnapshots(sourceCode) {
     ...
     * @returns The number of snapshots found
     */
    countSauceLabsSnapshots(sourceCode) {
        const sauceVisualCheckRegex = /(\w+)\.sauce_visual_check\s*\(\s*["']([^"']+)["']\s*\)/g;
...
exports.PythonCodeTransformer = PythonCodeTransformer;
//# sourceMappingURL=PythonCodeTransformer.js.map